...
class Level1(Background):
    ...
    def setup_boxes(self):
        self.coin_boxes.add(CoinBox(screen=self.screen, x=250 * BG_SCALER,
                                    y=135 * BG_SCALER, group=self.coin_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=329 * BG_SCALER,
                                    y=135 * BG_SCALER, group=self.coin_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=346 * BG_SCALER,
                                    y=71 * BG_SCALER, prize=ONEUPMUSHROOM, group=self.shroom_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=362 * BG_SCALER,
                                    y=135 * BG_SCALER, group=self.powerup_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=1242 * BG_SCALER,
                                    y=136 * BG_SCALER, prize=FLOWER, group=self.powerup_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=1690 * BG_SCALER,
                                    y=135 * BG_SCALER, group=self.coin_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=1737 * BG_SCALER,
                                    y=135 * BG_SCALER, group=self.coin_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=1785 * BG_SCALER,
                                    y=135 * BG_SCALER, group=self.coin_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=1737 * BG_SCALER,
                                    y=72 * BG_SCALER, group=self.coin_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=2058 * BG_SCALER,
                                    y=72 * BG_SCALER, group=self.coin_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=2074 * BG_SCALER,
                                    y=72 * BG_SCALER, group=self.coin_group))
        self.coin_boxes.add(CoinBox(screen=self.screen, x=2715 * BG_SCALER,
                                    y=135 * BG_SCALER, group=self.coin_group))
